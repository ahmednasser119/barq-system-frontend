...
export default function WhoAreWeSection() {
    ...
    return (
        <section
            ref={containerRef}
            className="px-[6%] pt-16 pb-32 bg-black "
        >
            <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                transition={{ duration: 0.6, delay: 1.2 }}
                className="absolute top-[30%] right-0 bottom-0 left-0  flex items-center justify-center z-50 ">
                <div className="relative w-full h-full  ">
                    <Image
                        ...
                        width={1660}
                        height={620}
                        className="object-contain  2xl:opacity-90   w-full blur-[200px] "
                    />
                </div>
            </motion.div>
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between w-full">
                            {stats.map((stat, index) => (
                                ...
                                >
                                    <div className="text-white text-[48px] mb-2 font-normal">
                                        <AnimatedCounter
                                            end={stat.value}
                                            prefix={stat.prefix}
                                            suffix={stat.suffix}
                                    ...
                                    <div className="text-[#C9C9C9] text-[16px] font-normal leading-tight">
                                        {stat.label}
                                    </div>
                                </motion.div>
                            ))}
